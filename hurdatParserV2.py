...
def getStormInfo(basin, lines, year):
    names = []
    ID = []
    years = []
    for x in range(len(lines)):
...
def makeDatabase(basin, lines):
    storms = getData(basin, lines, np.arange(yearsUsed[0], yearsUsed[1]))
    names, ID, years = getStormInfo(basin, lines, np.arange(yearsUsed[0], yearsUsed[1]))
...
class database():
    def __init__(self, basin):
        ...
        self.dataset = makeDatabase(basin, lines)

    def retrieveStorm(self, storm = None):
...
# hurdat2AL = database('AL')
# # hurdat2EP = database('EP')

# storm = hurdat2AL.retrieveStorm(['Debby', '2024'])
# ax = storm['track'](storm)
# # storm = hurdat2EP.retrieveStorm(['Orlene', '1974'])
# # ax = storm['track'](storm, ax)
# plt.show()
# plt.close()
