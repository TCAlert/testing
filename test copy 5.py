...
def map(interval, labelsize):
    ...
    ax.add_feature(cfeature.COASTLINE.with_scale('50m'), edgecolor = 'blue', linewidth = 0.5)
    ax.add_feature(cfeature.BORDERS.with_scale('50m'), edgecolor = 'blue', linewidth = 0.5)
    ax.add_feature(cfeature.STATES.with_scale('50m'), edgecolor = 'blue', linewidth = 0.5)
...
ax = map(5, 9)
a = test['unknown'].isel(valid_time = 0)
c = plt.pcolormesh(a.longitude, a.latitude, a.values, cmap = cmap.dust2(), vmin = 0, vmax = 7.5/1e4)
ax.set_title(f'GEFS Column-Integrated Mass Density (kg/m^2)\nInitialization: {year}-{str(month).zfill(2)}-{str(day).zfill(2)} at {str(hour).zfill(2)}00 UTC', fontweight='bold', fontsize=9, loc='left')
ax.set_title(f'Forecast Hour: 0', fontsize=9, loc='center') 
ax.set_title(f'Deelan Jariwala', fontsize=9, loc='right')
cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
cbar.ax.tick_params(axis='both', labelsize=9, left = False, bottom = False)
plt.savefig(r"C:\Users\deela\Downloads\salchart.png", dpi = 400, bbox_inches = 'tight')
#plt.show()
...
cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
cbar.ax.tick_params(axis='both', labelsize=9, left = False, bottom = False)
plt.savefig(r"C:\Users\deela\Downloads\salhov.png", dpi = 400, bbox_inches = 'tight')
plt.show()