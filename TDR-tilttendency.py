...
print(f'\n{"Case":10s}{"Name":15s}{"Start":20s}{"End":20s}{"Elapsed":10s}{"Start Tilt":15s}{"Final Tilt":15s}{"Tilt Change":15s}{"RMW Refl.":15s}{"RMW VVel.":15s}')
    ...
    while x < len(times):
        try:
            ...
            while y < len(times):
                ...
                if tChange >= -5 and delta >= 6 and delta <= 18 and iTilts[counter][x] > 10:
                    data = [0, 0]#getData(dataset, ['swath_reflectivity', 'swath_vertical_velocity'], [3, [5, 5.5, 6, 6.5, 7, 7.5, 8]], iCases[counter][x])

                    print(f'{str(iCases[counter][x])[:-2]:5s}{str(iCases[counter][y])[:-2]:5s}{str(iNames[counter][x].split('_')[0]):15s}{str(times[x]):20s}{str(times[y]):20s}{str(delta):10s}{str(iTilts[counter][x]):15s}{str(iTilts[counter][y]):15s}{str(tChange):15s}{str(round(data[0], 1)):15s}{str(round(data[1], 1)):15s}')
