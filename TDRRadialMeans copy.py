...
def binFunc(t, height):
    ...
    data1 = dataset1['swath_relative_vorticity'].sel(num_cases = l1, height = np.arange(0, 10.5, .5))
    data2 = dataset2['swath_relative_vorticity'].sel(num_cases = l2, height = np.arange(0, 10.5, .5))
    ...
    for x in range(len(bins) - 1):
        ...
        binsMean.append(np.nanmean(binContents, axis = (0, 1)))      
        binsMin.append(np.nanpercentile(binContents, 33, axis = (0, 1)))      
        binsMax.append(np.nanpercentile(binContents, 66, axis = (0, 1)))  
    return np.array(binsMean).squeeze(), np.array(binsMin).squeeze(), np.array(binsMax).squeeze(), data1.height#range(len(bins) - 1)
...
ax.set_ylabel('Height', weight = 'bold', size = 9)
ax.set_xlabel('Vorticity (1/s)', weight = 'bold', size = 9)
# ax.set_ylim(0, 0.003)

print(binsMean)

ax.plot(binsMean, bins, linewidth = 2.5, color = '#202040')
ax.fill_betweenx(bins, binsMin, binsMax, color='#8f8fbf', label = 'Aligning')    
ax.scatter(binsMean, bins, color = '#202040', zorder = 10)

ax.plot(binsMean2, bins, linewidth = 2.5, color = '#204020')
ax.fill_betweenx(bins, binsMin2, binsMax2, color='#8fbf8f', label = 'Misaligning')    
ax.scatter(binsMean2, bins, color = '#204020', zorder = 10)

plt.legend()

ax.set_title(f'TC-RADAR: Relative Vorticity Profile\n5km Radius', fontweight='bold', fontsize=10, loc='left')
ax.set_title('Deelan Jariwala', fontsize=10, loc='right')  
plt.savefig(r"C:\Users\deela\Downloads\verticalProfileVelocity.png", dpi = 400, bbox_inches = 'tight')
