...
def std(month, years, sd = False):
    ...
    allData = np.std(np.array(allData), axis = 0)

    labelsize = 8 
    ax = map(20, labelsize)    
    # ax.set_extent([220, 359, 0, 70])

    plt.contourf(data.lon, data.lat, allData, origin='lower', levels = np.arange(0, 2, .01), cmap = cmap.probs4(), extend = 'both', transform=ccrs.PlateCarree(central_longitude=0))
    plt.title(f'ERSSTv5 {title}\n30-Year Sliding Climatology' , fontweight='bold', fontsize=labelsize, loc='left')
    plt.title(f'{helper.numToMonth(month)} {str(years)}', fontsize = labelsize, loc = 'center')
    plt.title('Deelan Jariwala', fontsize=labelsize, loc='right')  
    cbar = plt.colorbar(orientation = 'vertical', aspect = 50, pad = .02)
    cbar.ax.tick_params(axis='both', labelsize=labelsize, left = False, bottom = False)
    plt.savefig(r"C:\Users\deela\Downloads\ersstCompositeSTD.png", dpi = 400, bbox_inches = 'tight')
    plt.show()
...
# analogs = [1876, 1887, 1901, 1915, 1919, 1927, 1928, 1932, 1949, 1952, 2001, 2012, 2013, 2017]
# analogs = '1876 1880 1882 1887 1901 1919 1928 1949 1962 1996 2000 2017'.split(' ')
# analogs = np.arange(2000, 2025, 1)
# analogs = '1882 1901 1928 1949 1952 1962 1996 2000 2001 2013'.split(' ')
# anomalies('9', analogs)#, True)
anomalies('2', [1957, 1965, 1991, 1997, 2009, 2015, 2023])
# std('1', [1958, 1973, 1992, 1998, 2016])