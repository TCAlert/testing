...
# open variable data
dataset = xr.open_dataset('http://psl.noaa.gov/thredds/dodsC/Datasets/ncep.reanalysis.derived/surface/slp.mon.mean.nc')
if extent[3] > 360:
...
data = dataset['slp'] * np.cos(np.radians(dataset['lat']))
...
for i in range(numOfEOFS):
    ...
    plt.savefig(r"C:\Users\deela\Downloads\slpEOF" + str(i + 1) + ".png", dpi = 400, bbox_inches = 'tight')
    plt.show()