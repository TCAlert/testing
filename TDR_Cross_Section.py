...
def getData(dataset, var, case):
    ...
    allData = dataset[var].sel(num_cases = case)
    offset = (np.pi / 2) + np.deg2rad(shd)
    levs = np.arange(0, 10.5, .5)
    datasets = []
    for x in range(len(levs)):
        temp = rePoPolar(allData.sel(level = levs[x]), offset)
        temp['r'] = temp['r'] / rmw
        rad = 4
        temp = temp['data'].sel(r = slice(0, rad))
        temp.values = np.flip(temp.values, axis = 1)
        newR = np.linspace(0, rad, 200)
        data = temp.interp(r = newR)
        datasets.append(data)
    data = xr.concat(datasets, dim = xr.DataArray(levs, dims = ['level']))#'level')

    # fig, ax = plt.subplots(subplot_kw={'projection': 'polar'})
    # ax.set_theta_zero_location("N")
    # ax.set_theta_direction(-1)
    # plt.pcolormesh(data.theta, data.r, data.sel(level = 3).values)
    # plt.show()
...
def makeComposites(dataset, list):
    ...
    for x in range(len(list)):
        dat, vmax = getData(dataset, 'swath_vertical_velocity', list[x])
...
if t == 'Decrease2':
    ...
    list1 = [488]#, 488]
    list2 = []#21, 26]
vvel1, wind1 = makeComposites(dataset1, list1)
vvel2, wind2 = makeComposites(dataset2, list2)
...
test = xr.concat(vvel, dim = 'cases')
print(test)
test = test.mean('cases')
test = test.interp(theta = np.linspace(-1 * np.pi, np.pi, 2000))
test = test.sel(r = slice(.75, 1.25)).mean('r')
tCoords = test.theta
print(test.shape)
# test.plot()
# plt.show()

print(np.array(vvel).shape)
valid_nums = np.count_nonzero(~np.isnan(np.mean(np.array(vvel), axis = 2)), axis = 0)
fig = plt.figure(figsize=(12, 9))
ax = plt.axes()
c = plt.pcolormesh(test.theta, test.level, valid_nums, cmap = cmap.probs2(), vmin = 0, vmax = np.nanmax(valid_nums))
...
plt.savefig(r"C:\Users\deela\Downloads\tdrcross_validcounts_" + t + ".png", dpi = 400, bbox_inches = 'tight')

#data = np.nanmean(vvel, axis = 0)
#data = np.where(valid_nums > (np.nanmax(valid_nums) / 2), data.values, np.nan)
fig = plt.figure(figsize=(12, 9))
ax = plt.axes()
c = plt.pcolormesh(test.theta, test.level, test, cmap = cmap.tempAnoms3())#, vmin = -2, vmax = 2)
cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
cbar.ax.tick_params(axis='both', labelsize=9, left = False, bottom = False)
ax.set_title(f'TC-RADAR: Normalized Tilt {t} Composite (>50% Valid Points)\nMean Vertical Velocity', fontweight='bold', fontsize=9, loc='left')
ax.set_title(f'Mean VMax: {str(int(meanWind))}kt\nDeelan Jariwala', fontsize=9, loc='right') 
plt.savefig(r"C:\Users\deela\Downloads\tdrcross_vvel_" + t + ".png", dpi = 400, bbox_inches = 'tight')
