...
dataset = xr.open_dataset(r"C:\Users\deela\Downloads\tc_radar_v3k_1997_2019_xy_rel_merge_ships.nc")
...
def panelPlot(dataset, caseNum):
    ...
    lats = data['merged_latitudes']
    lons = data['merged_longitudes']
    date = f'{str(data['merge_year'].values)}-{str(data['merge_month'].values).zfill(2)}-{str(data['merge_day'].values).zfill(2)} at {str(data['merge_hour'].values).zfill(2)}{str(data['merge_min'].values).zfill(2)}z'
    ...
    cr = axes[1].pcolormesh(lons, lats, data['merged_reflectivity'].values, cmap = cmap.reflectivity(), vmin = 0, vmax = 50, transform=ccrs.PlateCarree(central_longitude=0))
    ...
    cw = axes[2].pcolormesh(lons, lats, data['merged_wind_speed'].values * 1.94384, cmap = cmap.wind(), vmin = 0, vmax = 160, transform=ccrs.PlateCarree(central_longitude=0))
    ...
    cv = axes[3].pcolormesh(lons, lats, data['merged_vertical_velocity'].values, cmap = cmap.tempAnoms3(), vmin = -5, vmax = 5, transform=ccrs.PlateCarree(central_longitude=0))
    ...
    vc = axes[4].pcolormesh(lons, lats, data['merged_relative_vorticity'].values, cmap = cmap.probs(), vmin = 0, vmax = 0.005, transform=ccrs.PlateCarree(central_longitude=0))
    axes[4].streamplot(lons, lats, data['merged_zonal_wind'].values, data['merged_meridional_wind'].values, linewidth = 1, density = 1, color = 'black', transform = ccrs.PlateCarree(central_longitude = 0))
    ...
    plt.savefig(r"C:\Users\deela\Downloads\\" + date + ".png", dpi = 400, bbox_inches = 'tight')
    #plt.show()
name = 'Dorian'
year = 2019
height = 3
data = retrieveStorm(name, year)
refl = data['merged_wind_speed'].sel(level = height) * 1.94384
...
for x in range(len(refl.num_cases)):
    ...
    lats = data['merged_latitudes'].sel(num_cases = refl.num_cases[x])
    lons = data['merged_longitudes'].sel(num_cases = refl.num_cases[x])
