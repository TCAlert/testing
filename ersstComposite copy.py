...
def timeseries(month, years, sd = False):
    dataset = ((xr.open_dataset('http://psl.noaa.gov/thredds/dodsC/Datasets/noaa.ersst.v5/sst.mnmean.nc')['sst']).sel(lat = slice(10, -10), lon = slice(220, 280))).mean(['lat', 'lon'])
    print(dataset)
    allData = []
    anomalies = []
    climos = []
    ...
    plt.title(f'ERSSTv5 Sea Surface Temperatures\n-10S to 10N, 140 to 80W' , fontweight='bold', fontsize=10, loc='left')
...
timeseries(month='1', years=range(1854, 2025))