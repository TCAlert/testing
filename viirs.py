...
def plot(files, geofs, ax, num = '5', cmp = 'irg'):
    for x in range(len(files)):
        if num == '1':
            file = xr.open_dataset(r"C:\Users\deela\Downloads\\" + files[x], group = '/All_Data/VIIRS-I1-SDR_All/', phony_dims = 'access', engine = 'h5netcdf')
            data = file['Reflectance']
            data = data.rename({'phony_dim_3' : 'phony_dim_0', 'phony_dim_4' : 'phony_dim_1'})
            factors = file['ReflectanceFactors']
        else:
            file = xr.open_dataset(r"C:\Users\deela\Downloads\\" + files[x], group = '/All_Data/VIIRS-I5-SDR_All/', phony_dims = 'access', engine = 'h5netcdf')
            data = file['BrightnessTemperature']
            factors = file['BrightnessTemperatureFactors']
        geof = xr.open_dataset(r"C:\Users\deela\Downloads\\" + geofs[x], group = '/All_Data/VIIRS-IMG-GEO_All/', phony_dims = 'access', engine = 'h5netcdf')

        data = data * factors[0] + factors[1]
        data = data.where((geof.Longitude != -999.3) & (geof.Latitude != -999.3), drop = True)
        geof = geof.where((geof.Longitude != -999.3) & (geof.Latitude != -999.3), drop = True)
        lons = geof['Longitude']
        lats = geof['Latitude']

        if num == '1':
            c = ax.pcolormesh(lons, lats, data, vmin = 0.2, vmax = 1.2, cmap = 'Greys_r')
        else:
            cmap, vmax, vmin = cmaps.irtables[cmp.lower()]
            c = ax.pcolormesh(lons, lats, data - 273, vmin = vmin, vmax = vmax, cmap = cmap)
...
ax = map(-91.039, 21.755, -2)
c = plot(files, geofs, ax, '1')
plt.title(f'{satt} VIIRS I01 Visible Imagery\nTime: {date} at {time}Z' , fontweight='bold', fontsize=labelsize + 1, loc='left')
plt.title(f'Deelan Jariwala', fontsize=labelsize + 1, loc='right')  
cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
plt.savefig(r"C:\Users\deela\Downloads\\" + satt + "_" + '_'.join(date.split('/')) + "_" + time + "_4.png", dpi = 500, bbox_inches = 'tight')
plt.show()