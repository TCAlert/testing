...
data, init = gfs.getData(['tmp2m'], np.datetime64(f'{date}T{str(hour).zfill(2)}'))
data[0].values = ((data[0].values - 273.15) * (9/5)) + 32
ax = map(5, labelsize)
ax.set_extent([-130, -60, 20, 55])
plt.contourf(data[0].lon, data[0].lat, data[0].values , cmap = cmap.temperature(), levels = np.arange(-100, 101, 1), extend = 'both')
plt.colorbar(orientation = 'vertical', aspect = 50, pad = .02)

plt.title(f'0.25\u00b0 GFS: 2m AGL Temperature (\u00b0F)\nInitialization: {init}', fontweight='bold', fontsize=labelsize, loc='left')
