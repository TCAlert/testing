...
yList = [[], [gShear], [dShear], [gShear, dShear], [gShear, dist, vmax, sst, mpi, rhlm, bdv, tmp, div, lat], [dShear, dist, vmax, sst, mpi, rhlm, bdv, tmp, div, lat], [gShear, dShear, dist, vmax, sst, mpi, rhlm, bdv, tmp, div, lat], [dist, vmax, sst, mpi, rhlm, bdv, tmp, div, lat]]

n = 1
for x in range(len(xList)):
    for y in range(len(yList)):
        print(f'{n}. {np.shape(np.array(xList[x] + yList[y]))}')
        try:
