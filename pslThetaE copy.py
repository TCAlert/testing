...
uwnd = (data[2].sel(level = level1)).squeeze()
uClm = (clim[2].sel(level = level1).mean('time')).squeeze()

vwnd = (data[3].sel(level = level1)).squeeze()
vClm = (clim[3].sel(level = level1).mean('time')).squeeze()

plt.imshow(stabData - stabClim, vmin = -10, vmax = 10, cmap = cmap.tempAnoms3())
plt.colorbar()
plt.show()

labelsize = 8 
ax = map(10, labelsize)    
ax.set_extent([240, 359, 0, 70], crs = ccrs.PlateCarree())
...
c = plt.contourf(stabData.lon, stabData.lat, stabData - stabClim, origin='lower', levels = np.arange(-10, 10.05, .05), cmap = cmap.tempAnoms3(), extend = 'both', transform=ccrs.PlateCarree(central_longitude=0))
plt.streamplot(uwnd.lon, vwnd.lat, uwnd.values - uClm.values, vwnd.values - vClm.values, linewidth = 1, density = 1, color = 'black', transform = ccrs.PlateCarree(central_longitude = 0))
plt.title(f'NCEP/NCAR Reanalysis I: {level1}mb Theta-E Anomalies\nClimatology: {climoYears[0]}-{climoYears[1]}' , fontweight='bold', fontsize=labelsize, loc='left')
plt.title(f'{helper.numToMonth(months[0])} {str(years[0])}', fontsize = labelsize, loc = 'center')
plt.title('2.5\u00b0\nDeelan Jariwala', fontsize=labelsize, loc='right')  
cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
cbar.ax.tick_params(axis='both', labelsize=labelsize, left = False, bottom = False)
plt.savefig(r"C:\Users\deela\Downloads\thetaepslAnomalies.png", dpi = 400, bbox_inches = 'tight')
plt.show()