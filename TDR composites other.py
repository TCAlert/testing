...
def makeComposites(dataset, list):
    ...
    for x in range(len(list)):
        dat, vmax = getData(dataset, ['swath_radial_wind', 'swath_radial_wind'], [3, [5, 5.5, 6, 6.5, 7, 7.5, 8]], list[x])
...
fig, ax = plt.subplots(subplot_kw={'projection': 'polar'}, figsize = (12, 9))
c = plt.pcolormesh(tCoords, refl[0].r, data, cmap = cmap.tempAnoms3(), vmin = -10, vmax = 10)
labels(ax)
cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
cbar.ax.tick_params(axis='both', labelsize=9, left = False, bottom = False)
ax.set_title(f'TC-RADAR: Normalized Tilt {t} Composite (>50% Valid Points)\n5-8km Mean Radial Wind', fontweight='bold', fontsize=9, loc='left')
ax.set_title(f'Mean VMax: {str(int(meanWind))}kt\nDeelan Jariwala', fontsize=9, loc='right') 
plt.savefig(r"C:\Users\deela\Downloads\TCTiltProject\tdrcomp_rad_" + t + ".png", dpi = 400, bbox_inches = 'tight')
...
cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
cbar.ax.tick_params(axis='both', labelsize=9, left = False, bottom = False)
ax.set_title(f'TC-RADAR: Normalized Tilt {t} Composite\n5-8km Mean Radial Wind Standard Deviation', fontweight='bold', fontsize=9, loc='left')
ax.set_title(f'Mean VMax: {str(int(meanWind))}kt\nDeelan Jariwala', fontsize=9, loc='right') 
plt.savefig(r"C:\Users\deela\Downloads\TCTiltProject\tdrcomp_radstd_" + t + ".png", dpi = 400, bbox_inches = 'tight')
