...
def plot(data, year, month, day, hour, level = '1000', name = None, t = 'wind'):
    ...
    print(data.sort_values(by = 'W (kts)', ascending = False))
    windDir = data['D']
    windSpd = data['W (kts)']
    seaLevP = data['SLP']
    airTemp = data['AT']
    ...
    plt.savefig(r"C:\Users\deela\Downloads\\doris\\" + name + title + str(year) + str(month).zfill(2) + str(day).zfill(2) + str(hour).zfill(2) + ".png", dpi = 400, bbox_inches = 'tight')
    # plt.savefig(r"C:\Users\deela\Downloads\sandy1.png", dpi = 400, bbox_inches = 'tight')
    # plt.show()
    plt.close()

data = pd.read_csv(r"C:\Users\deela\Downloads\AL081975 - datav2.csv")
for x in list(np.arange(4, 5)):
    for y in range(0, 24, 6):
        year, month, day, hour = '1975', '9', str(x), y
        level = 1000

        try:
            plot(data, year, month, day, hour, level, 'Doris', 'temp')
