...
def run(satellite, date, time, lat, lon, band, cmp = None, zoom = 2, num = 0):
    ...
    plt.savefig(r"C:\Users\deela\Downloads\goesloop\\05july_" + str(counter) + ".png", dpi = 150, bbox_inches = 'tight')
    #plt.show()
    plt.close()
    dataset.close()
...
counter = 160
for x in range(21, 26, 1):
    for y in range(0, 24, 3):
        try:
            counter = counter + 1
            run(12, f"07/{str(x)}/2005", f"{str(y).zfill(2)}00", "0", "0", "3", "wv", 'watl', counter)
        except:
            pass